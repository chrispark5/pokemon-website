...
const TypeMatchup = () => {
  ...
  const weaknesses = damage_relations.double_damage_from.map(type => type.name);
  ...
  return (
    <Container>
      ...
      <Text align="center" size="lg">
        <strong>Strengths:</strong> {strengths.map(type => (
          <img
            key={type}
            src={typeIcons[type]}
            alt={`${type} icon`}
      ...
      <Text align="center" size="lg" mt="md">
        <strong>Weaknesses:</strong> {weaknesses.map(type => (
          <img
            key={type}
            src={typeIcons[type]}
            alt={`${type} icon`}
